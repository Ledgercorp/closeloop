...
- The original `closeloop-sage.vercel.app` value was not the project production alias. Vercel's
  actual production alias is `closeloop-zeta.vercel.app`; generated deployment URLs remain
  protected, while the production alias is signed-out accessible.
- Added a secret-free static `/demo/` bundle generated by
  `scripts/build_proof_card_validation.py --public-bundle`. It contains only the index,
  confirmation, three canonical outcome cards, and manifest. The build-time temporary database,
  test signing secret, and compact attestations are not copied into the bundle.
- Mounted the public assets before the authenticated MCP catch-all and kept every consequential MCP
  action bearer-gated. Root JSON now links to `/demo/`; `/mcp` authentication behavior is unchanged.
- Selected Python 3.12 for the current Vercel Python runtime and included the static assets in the
  wheel. No Vercel project environment variables or production secrets were added.
- Targeted demo/MCP tests: `12 passed`; complete final suite after executable changes: `187 passed`;
  dependency lock, compile, wheel-content, leak/path, traversal, and `git diff --check` checks passed.
- Verification scope: **VERIFIED** public deterministic judge demo and browser rendering;
  **INTEGRATION VERIFIED** Alexa+/MCP locally; **SIMULATED** AWS/DynamoDB; no live Alexa+, live AWS,
  real provider, production OAuth/account linking, or Alexa+ host proof-card rendering claimed.

Deployment validation commands:

```bash
PYTHONPATH=src uv run --no-editable python \
  scripts/build_proof_card_validation.py src/closeloop/public_demo --public-bundle
PYTHONPATH=src uv run --no-editable pytest -q \
  tests/test_demo_submission.py tests/test_mcp_server.py
PYTHONPATH=src uv run --no-editable pytest -q
curl -i https://closeloop-zeta.vercel.app/
curl -i https://closeloop-zeta.vercel.app/health
curl -i https://closeloop-zeta.vercel.app/demo/
curl -i https://closeloop-zeta.vercel.app/demo/healthy.html
curl -i -X POST https://closeloop-zeta.vercel.app/mcp \
  -H 'Content-Type: application/json' \
  --data '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-11-25","capabilities":{},"clientInfo":{"name":"public-probe","version":"1"}}}'
git diff --check
...
- Milestone 8 package suite: `185 passed`; deployment-fix suite after two added tests: `187 passed`,
  with the unchanged non-failing Starlette/anyio deprecation warning;
...
- Confirm the final GitHub repository is public, its Apache-2.0 license is visible in About, and its
  homepage points to `https://closeloop-zeta.vercel.app/demo/`.
...
- The public deterministic demo is safe to submit. Do not present it as live provider execution. A
  live public service still requires shared production storage, OAuth/account linking, and a trusted
  confirmation authority; test/recording keys must never be deployed.

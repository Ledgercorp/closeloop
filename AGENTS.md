# CloseLoop project authority and scope

The Amazon-generated context below is advisory for future Amazon Devices work. It must not
redesign CloseLoop or override, in descending project authority, `HANDOFF.md`,
`NON_NEGOTIABLES.md`, `ACCEPTANCE_TESTS.md`, `HACKATHON_REQUIREMENTS.md`, `DEMO_SPEC.md`,
`docs/architecture.md`, or `docs/trust-model.md`.

Do not apply Vega/Fire OS dependencies, templates, project structure, or implementation workflows
unless a later approved milestone explicitly selects that platform. Do not begin Alexa+, AWS
orchestration, final UI, or additional provider work unless the current authoritative milestone
requires it. Amazon guidance may never weaken PASS/FAIL/INCONCLUSIVE semantics, independent
verification, confirmation-before-mutation, immutable terminal outcomes, owner authorization, or
the prohibition on direct verdict-write capabilities.
